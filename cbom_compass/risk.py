...
def find_hybrid_partners(assets: list[Asset]) -> dict[str, str]:
    ...
    Co-location in one module is the available signal: a hybrid is built by
    running both halves together, so `kex_mlkem.py` holds the ML-KEM and the
    X25519 sides of `mlkem768x25519-sha256`. It is a heuristic and it is stated
    as one in the rationale, because nothing in the source says "this is a
    hybrid" — but pairing like with like (a KEM covers a key agreement, not a
    hash) keeps it from firing on a module that merely mentions both.
    """
    ...
    for asset in assets:
        if asset.algorithm in kb.PQC:
            continue
        classical_role = kb.PRIMITIVE.get(asset.algorithm)
        available = pqc_by_module.get(_module_of(asset), {})
